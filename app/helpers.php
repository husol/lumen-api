...
function buildInsertUpdateSQL($tablename, $dataObject = array(), $fields = array(), $exclude = false)
{
    if (empty($tablename)) {
        return array('result' => false, 'message' => 'Table name must be inputted.');
    }
    if (empty($fields)) {
        return array('result' => false, 'message' => 'Fields for updating/not updating must be inputted.');
    }
    if (empty($dataObject)) {
        return array('result' => false, 'message' => 'No data updated.');
    }
...
function trimApiRootUrl($url)
{
    ...
    $s = in_array($prefix, array('http://', 'https://')) ? '' : '.';
...
function convertToDateDisplay($time, $detail = false)
{
    if ($detail) {
